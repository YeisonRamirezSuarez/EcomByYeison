...
const RootLayout = async ({ children }: { children: React.ReactNode }) => {
  ...
  // CSP nonce set by proxy.ts; applied to the inline theme-boot script below so
  // it's allowed under the strict-dynamic policy.
    ...
    <html lang={locale} className={poppins.variable}>
      <head>
        ...
        {/* iOS launch images — not supported by Next's Metadata API, so the
            apple-touch-startup-image links are emitted manually. Generated by
            `npm run splash`. */}
        {splashScreens.map((s) => (
          <link
            key={s.href}
